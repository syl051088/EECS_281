...
cost best_price(const std::vector<cost>& prices) {
    ...
    vector<vector<cost>>v;
    v.resize(prices.size(), vector<cost>(6, numeric_limits<cost>::max()/4));
    v[0][0] = discounted(prices[0]);
    v[0][1] = prices[0];

    for (uint32_t i = 1; i < prices.size(); ++i) {
        v[i][0] = std::min(v[i - 1][5], v[i - 1][0] + discounted(prices[i]));
        for (uint32_t j = 1; j <= 5; ++j) {
            v[i][j] = std::min(v[i - 1][j - 1] + prices[i], v[i - 1][j] + discounted(prices[i]));
        }
    }
    return v.back()[0];
}
